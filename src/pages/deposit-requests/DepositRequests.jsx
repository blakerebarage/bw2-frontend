...
const DepositRequests = () => {
  ...
  const fetchDepositRequests = async () => {
    try {
      setLoading(true);
      const response = await axiosSecure.get(`/api/v1/finance/all-recharge-request-by-user?page=${currentPage}&search=${search}`);
      ...
      if (response.data.success) {
        ...
        addToast(response.data.message || "Failed to fetch deposit requests", {
          appearance: "error",
          autoDismiss: true,
        });
      }
      ...
      addToast("Failed to fetch deposit requests", {
        appearance: "error",
        autoDismiss: true,
      });
    } finally {
      ...
      <div className="px-4 py-6 max-w-lg mx-auto space-y-4">
        ...
        <div className="bg-[#1a1f24] rounded-lg shadow-sm border border-[#facc15]/20 p-6">
          <div className="text-center">
            ...
            <h1 className="text-lg font-medium text-gray-300 mb-2">{t('depositRequests') || 'Deposit Requests'}</h1>
            <p className="text-sm text-gray-400">{t('viewYourDepositRequests') || 'View all your deposit requests and their status'}</p>
          </div>
        </div>
          ...
          <div className="bg-[#1a1f24] rounded-lg shadow-sm border border-[#facc15]/20 p-4">
            <div className="text-center">
              ...
              <p className="text-lg font-semibold text-green-400">
                {depositRequests.filter(req => req.status === 'approved').length}
              </p>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-[#1a1f24] rounded-lg shadow-sm border border-[#facc15]/20 p-8">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-gray-300 mb-2">{t('noDepositRequests') || 'No deposit requests found'}</h3>
              <p className="text-gray-400">{t('noDepositRequestsDesc') || 'You haven\'t made any deposit requests yet'}</p>
            </div>
          </div>
        )}

        {/* Pagination */}
        {!loading && depositRequests.length > 0 && totalPages > 1 && (
          <div className="bg-[#1a1f24] rounded-lg shadow-sm border border-[#facc15]/20 p-4">
