...
const WalletAgentDepositRequests = ({ 
  ...
  const handleRequestAction = async (requestId, action) => {
    try {
      const res = await axiosSecure.patch(`/api/v1/finance/update-recharge-request-status-by-wallet-agent/${requestId}`, {
    ...
    <div>
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="bg-green-500/20 text-green-400 px-3 py-1 rounded-full text-sm">
          {depositRequests.length} total
        </span>
        <span className="bg-yellow-500/20 text-yellow-400 px-3 py-1 rounded-full text-sm">
          {depositNotificationCount} pending
        </span>
      </div>
...
export default WalletAgentDepositRequests; 